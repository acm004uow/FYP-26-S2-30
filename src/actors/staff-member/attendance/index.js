      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="my-6 rounded-xl border bg-white shadow-sm p-5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              {scannerMessage && <p className="text-xs text-emerald-600 mt-1">{scannerMessage}</p>}
            </div>
          </div>
            ...
            <div className="flex items-center gap-4 sm:pl-4">
              ...
              <div>
                <button
                  ...
                  className="flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-700 hover:bg-emerald-800 px-5 py-2.5 text-sm font-semibold text-white transition"
                >
        ...
        <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
          ...
          {viewMode === 'calendar' ? (
                ...
                {calendarWeeks.map((week, weekIndex) => (
                  <div key={weekIndex} className="grid grid-cols-7 gap-2">
                    {week.map(date => {
                      ...
                      const dotColor = status === 'completed' ? 'bg-emerald-500' : status === 'in_progress' ? 'bg-amber-400' : 'bg-transparent'
            ...
            <>
              <div className="hidden grid-cols-[1.2fr_1.6fr_1fr] gap-4 border-b border-gray-100 bg-emerald-50 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-emerald-800 md:grid">
                <span>Date</span>
                    ...
                    <div key={record.work_date} className="grid grid-cols-[1.2fr_1.6fr_1fr] items-center gap-4 px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          {relativeLabel && (
                            <p className={`text-xs ${relativeLabel === 'Today' ? 'text-emerald-600 font-medium' : 'text-gray-400'}`}>{relativeLabel}</p>
                          )}
                        </div>
                      </div>
                        ...
                        {!clockedIn ? (
                          ...
                          <>
                            ...
                            <span className={`mt-1 inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                              status === 'completed' ? 'bg-emerald-100 text-emerald-700' : 'bg-emerald-50 text-emerald-600'
                            }`}>
              ...
              {!loading && records.length > 0 && (
                <p className="flex items-center justify-center gap-1.5 border-t border-gray-100 py-3 text-xs text-emerald-700">
                  <Clock className="h-3.5 w-3.5" />
