    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Browse the shared staff pool, their workload, and performance.</p>
        </div>
      </div>
            ...
            <div className="absolute left-0 z-10 mt-1 w-48 rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
              {departmentOptions.map(option => (
                <button
                  ...
                  className={`w-full rounded-md px-3 py-2 text-left text-sm hover:bg-accent-100 ${departmentFilter === option ? 'font-semibold text-accent-600' : 'text-gray-700'}`}
                >
            ...
            <div className="absolute left-0 z-10 mt-1 w-40 rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
              {STATUS_FILTERS.map(option => (
                <button
                  ...
                  className={`flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm hover:bg-accent-100 ${statusFilter === option ? 'font-semibold text-accent-600' : 'text-gray-700'}`}
                >
            ...
            <div className="flex items-center gap-1.5">
              ...
              {Array.from({ length: totalPages }, (_, index) => index + 1).map(pageNumber => (
                <button
                  ...
                  className={`flex h-8 w-8 items-center justify-center rounded-lg border text-sm font-medium ${pageNumber === safePage ? 'border-accent bg-accent text-white' : 'border-gray-200 bg-white text-gray-500 hover:bg-gray-50'}`}
                >
                  {pageNumber}
                </button>
              ))}
                ...
                <div className="absolute right-0 z-10 mt-1 w-28 rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
                  {PAGE_SIZE_OPTIONS.map(option => (
                    <button
                      ...
                      className={`w-full rounded-md px-3 py-2 text-left text-sm hover:bg-accent-100 ${pageSize === option ? 'font-semibold text-accent-600' : 'text-gray-700'}`}
                    >
                      {option} per page
                    </button>
                  ))}
