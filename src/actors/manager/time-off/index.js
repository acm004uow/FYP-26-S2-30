...
export default function ManagerTimeOff() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      requestChannel = supabase
        .channel(`manager-time-off-${user.id}-${Date.now()}`)
        .on(
          'postgres_changes',
  ...
  const handleSubmit = async (event) => {
    ...
    // host_admin_id always resolves to the owner's own profile id, so this notifies the owner
    // directly with no extra lookup needed.
    ...
    await supabase.from('audit_logs').insert({
      ...
      details: `${profile.full_name} requested ${summary}`,
    })
