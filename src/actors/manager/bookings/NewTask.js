...
export default function ManagerNewTask({
  ...
  const handleClearAll = () => {
    setForm(prev => ({ ...emptyForm, scheduledDate: initialDate || '', serviceType: serviceTypes.includes(emptyForm.serviceType) ? emptyForm.serviceType : serviceTypes[0] }))
    setCoordinates(null)
    setSelectedStaffId('')
    setGenerateError('')
    setAiHint('')
  ...
  // "AI Agent" tab: turns a shorthand note (e.g. "Cus Name - Saung, Location - 123456 #05-01,
  // tomorrow 2pm") into the same form fields the Manual tab uses, so the existing AI staff
  // recommendation (the useEffect below, keyed off form.location/estimatedHours/etc.) and staff
  // picker keep working unchanged — the AI agent still fills in and recommends staff, it's just
  // a faster way to get there than typing into each field by hand.
  const handleParseAi = async () => {
    ...
    try {
      ...
      setForm(prev => ({
        ...prev,
        guestName: result.customerName || prev.guestName,
        ...
        <form onSubmit={handleCreateTask} className="bg-white rounded-2xl shadow-sm border p-6 space-y-5">
          ...
          <div className="flex gap-1 border-b">
            <button
              ...
              className={`px-4 py-2 text-sm font-semibold transition ${createMode === 'manual' ? 'border-b-2 border-accent text-accent-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
              Manual
            </button>
            <button
              ...
              className={`flex items-center gap-1 px-4 py-2 text-sm font-semibold transition ${createMode === 'ai' ? 'border-b-2 border-accent text-accent-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
          ...
          {createMode === 'ai' && (
            <div className="rounded-lg border border-purple-100 bg-purple-50 p-3">
              ...
              <textarea
                ...
                rows={2}
                className="mt-1 w-full rounded-lg border border-gray-200 bg-white p-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500"
              />
