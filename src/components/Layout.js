...
function getBotResponse(userInput, role) {
  ...
  if (role === 'manager') {
    if (input.includes('report')) return "Daily: 24 tasks, 18 completed. Weekly: 142 tasks. Monthly: 560 tasks."
    if (input.includes('allocation')) return "2 pending task requests awaiting review."
    return "I can help with reports, task requests, or staff overview."
  }
  if (role === 'department') {
    if (input.includes('task')) return "You have 1 pending request. #R101 under review."
    if (input.includes('available')) return "6 staff available today."
    return "Create a new task request or check your submitted tasks."
  }
  if (role === 'staffMember') {
    if (input.includes('task')) return "You have 2 active tasks today."
    if (input.includes('availability')) return "Your status is 'Available'."
...
export default function Layout({ children, role = 'manager' }) {
  ...
  const sampleNotifications = [
    { id: 1, message: 'New task assigned: Equipment Check', time: '10:30 AM' },
    { id: 2, message: 'Staff availability changed: John Smith is now Available', time: '9:15 AM' },
    { id: 3, message: 'Task R101 approved by manager', time: 'Yesterday' },
  ]

  useEffect(() => {
    // Preload sample notifications
