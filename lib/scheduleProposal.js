...
import { fetchSupabaseRows, patchSupabaseRow } from './supabaseRest'
...
export async function buildScheduleProposal(hostAdminId, { start_date, end_date }) {
  ...
  const proposal = bookings.map((booking) => {
    const base = {
    ...
    if (topMatch) {
      ...
      // Persist the recommendation onto the booking row itself, same as the customer one-off
      // booking flow does at creation time (src/actors/customer/book/index.js) — otherwise a
      // recurring-generated visit (which is created bare, with no recommendation, by
      // generateWeeklyVisits above) would only ever show its AI match inside this ephemeral
      // proposal response and never in the manager's Bookings for Review table, leaving it
      // stuck on "Awaiting assignment" until someone manually re-runs the match.
      pendingWrites.push(patchSupabaseRow('bookings', booking.id, {
