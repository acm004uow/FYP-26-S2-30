...
export async function buildScheduleProposal(hostAdminId, { start_date, end_date }) {
  ...
  const [bookings, staffRows, systemParamsRows, approvedTimeOff] = await Promise.all([
    fetchSupabaseRows('bookings', [
      ['select', 'id,service_type,location,latitude,longitude,scheduled_date,scheduled_time,estimated_hours,status,assigned_staff_id,description,notes,recurring_booking_id,staff_profiles(staff_name)'],
    ...
    fetchSupabaseRows('staff_profiles', [
      ['select', 'id,staff_name,assigned_region,latitude,longitude,availability,current_workload,weekly_working_hours,max_weekly_hours,performance_rating,status,is_suspended'],
      ['host_admin_id', `eq.${hostAdminId}`],
  ...
  // Seeds conflict-checking with every already-assigned booking in the week, then grows as this
  // batch itself assigns new ones below — so two different (possibly unrelated) bookings on the
  // same day/time can never both land on the same staff member, which is how "David" used to show
  // up twice on the same slot in the Schedule grid.
  const runningAssignments = bookings
    .filter((b) => b.assigned_staff_id)
    .map((b) => ({ id: b.id, assigned_staff_id: b.assigned_staff_id, scheduled_date: b.scheduled_date, scheduled_time: b.scheduled_time, estimated_hours: b.estimated_hours }))
  ...
  return bookings.map((booking) => {
    ...
    if (topMatch) {
      slotState?.used.add(topMatch.staff_id)
      runningAssignments.push({ id: booking.id, assigned_staff_id: topMatch.staff_id, scheduled_date: booking.scheduled_date, scheduled_time: booking.scheduled_time, estimated_hours: booking.estimated_hours })
    }
    return {
      ...base,
