...
// A company that's linked a real Google Business Profile (see MarketingPanel.js) ranks on that
// live-synced public rating instead of in-app reviews — it's the number actual customers already
// trust and search by, and it doesn't require this company to have any bookings in our system yet.
// Falls back to in-app booking_feedback ratings for companies that haven't linked one.
function scoreCompany(company, ratingInfo) {
  if (typeof company?.google_rating === 'number' && (company?.google_rating_count || 0) > 0) {
    const count = company.google_rating_count
    const experienceBonus = Math.min(count, EXPERIENCE_CAP)
    const score = company.google_rating * 20 + experienceBonus * 2
    const reason = `${company.google_rating.toFixed(1)}★ on Google from ${count} review${count === 1 ? '' : 's'}`
    return { score, reason, rating: { average: company.google_rating, count, source: 'google' } }
  }

  if (!ratingInfo || ratingInfo.count === 0) {
    return { score: 0, reason: 'No reviews yet for this service type', rating: null }
  }
  ...
  return { score, reason, rating: { ...ratingInfo, source: 'internal' } }
}
...
export function rankCompaniesForServiceType(companies, ratings) {
  return companies
    .map(company => {
      ...
      const { score, reason, rating } = scoreCompany(company, ratingInfo)
      return { ...company, score, rating, reason }
    })
    .sort((a, b) => {
