...
// booking (e.g. an unexpected job) directly from the Weekly Schedule grid.
export async function createManualBooking({ hostAdminId, serviceType, location, latitude, longitude, description, estimatedHours, scheduledDate, scheduledTime, staff, managerUserId }) {
  ...
  const { data: createdBooking, error } = await supabase
    ...
    .insert({
      ...
      assigned_staff_id: staff.id,
      reviewed_by: managerUserId,
      recommendation_reason: 'Manually created by manager',
    })
