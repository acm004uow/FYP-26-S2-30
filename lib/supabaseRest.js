...
// Bulk PATCH by arbitrary filter (e.g. host_admin_id), unlike patchSupabaseRow below which
// targets exactly one row by id.
export async function patchSupabaseRows(table, paramPairs, patch) {
  const { url, key } = getSupabaseConfig()
  const requestUrl = new URL(`${url}/rest/v1/${table}`)
  paramPairs.forEach(([name, value]) => requestUrl.searchParams.append(name, value))
  const response = await fetch(requestUrl, {
