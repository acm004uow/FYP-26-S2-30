...
export default function ManagerStaffPage() {
  ...
  function edit(row) { setEditing(row.id); setActionRow(null); setForm({ ...row, skills: (row.skills || []).join(", ") }); }
  async function suspend(row) { await supabase.from("staff_profiles").update({ is_suspended:true, status:"suspended", availability:"unavailable" }).eq("id", row.id); await supabase.from("audit_logs").insert({ action:"suspend_staff_profile", details: row.staff_name }); setActionRow(null); load(); }
  ...
  <div className="card"><table><thead><tr><th>Name</th><th>Skills</th><th>Region</th><th>Availability</th><th>Workload</th><th>Status</th><th>Action</th></tr></thead><tbody>{filtered.map(s=>{
    const isActionRow = actionRow === s.id;
    return <tr key={s.id}><td>{s.staff_name}<br/><span className="small">{s.email}</span></td><td>{(s.skills||[]).join(", ")}</td><td>{s.assigned_region}</td><td>{s.availability}</td><td>{s.current_workload} tasks / {s.weekly_working_hours} hrs</td><td><StatusBadge value={s.status || (s.is_suspended ? "suspended" : "active")} /></td><td className="row">{isActionRow ? <><button className="btn secondary" onClick={()=>edit(s)}>Update</button><button className="btn danger" onClick={()=>suspend(s)}>Suspend</button><button className="btn secondary" onClick={()=>setActionRow(null)}>Cancel</button></> : <button className="btn secondary" onClick={()=>setActionRow(s.id)}>Edit</button>}</td></tr>;
  })}</tbody></table></div></main></RequireRole>;
}
